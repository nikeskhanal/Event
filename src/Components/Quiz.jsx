...
const Quiz = () => {
  const [questions, setQuestions] = useState([]); // To store the list of questions
  const [currentQuestion, setCurrentQuestion] = useState('');
  const [options, setOptions] = useState(['', '', '', '']); // Array for 4 options
  const [correctAnswer, setCorrectAnswer] = useState(''); // To store correct answer index as a number
  ...
  // Handle changes to option inputs
  ...
  const handleAddQuestion = () => {
    if (
      ...
      setCurrentQuestion(''); // Reset the question
      setOptions(['', '', '', '']); // Reset the options
      setCorrectAnswer(''); // Reset the correct answer
    } else {
